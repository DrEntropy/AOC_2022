...
find_distance <- \(start_point) {
      ...
      while(any(current != end)){
          ...
          north <- current + c(1,0)
          south <- current - c(1,0)
          if(all(east <= dim(distances)) && height_matrix[east] <= height_matrix[current] +1 ){
            distances[east] <- min(distances[east],distances[current]+1)
          }
          if(all(west > c(0,0)) && height_matrix[west] <= height_matrix[current] +1 ){
            distances[west] <- min(distances[west],distances[current]+1)
          }
          if(all(north <= dim(distances)) && height_matrix[north] <= height_matrix[current] +1 ){
            distances[north] <- min(distances[north],distances[current]+1)
          }
          if(all(south > c(0,0)) && height_matrix[south] <= height_matrix[current] +1 ){
            distances[south] <- min(distances[south],distances[current]+1)
          }
          temp <- distances
          temp[visited] <- Inf
